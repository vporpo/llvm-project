...
void Value::replaceUsesWithIf(
    Value *OtherV,
    llvm::function_ref<bool(User *DstU, unsigned OpIdx)> ShouldReplace) {
  assert(getType() == OtherV->getType() && "Can't replace with different type");
  llvm::Value *OtherVal = OtherV->Val;
  auto &Tracker = getTracker();
  Val->replaceUsesWithIf(
      OtherVal, [&ShouldReplace, &Tracker, this](llvm::Use &U) -> bool {
        User *DstU = cast_or_null<User>(Ctx.getValue(U.getUser()));
        if (DstU == nullptr)
          return false;
...
Argument::Argument(llvm::Argument *Arg, Context &SBCtx)
    : Value(ClassID::Argument, Arg, SBCtx) {}

User::User(ClassID ID, llvm::Value *V, Context &SBCtx) : Value(ID, V, SBCtx) {}
...
BBIterator &BBIterator::operator++() {
  ...
  if (It == ItE)
    return *this;
  Instruction &NextI = *cast<Instruction>(SBCtx->getValue(&*It));
...
llvm::Instruction *Instruction::getTopmostIRInstruction() const {
...
void Instruction::insertBefore(Instruction *BeforeI) {
  ...
  llvm::Instruction *BeforeTopI = BeforeI->getTopmostIRInstruction();
  auto IRInstrs = getLLVMInstrs();
...
Constant::Constant(llvm::Constant *C, Context &SBCtx)
    : User(ClassID::Constant, C, SBCtx) {}
...
Value *CmpInst::create(llvm::CmpInst::Predicate Pred, Value *LHS, Value *RHS,
                       Instruction *InsertBefore, Context &SBCtx,
                       const Twine &Name, MDNode *FPMathTag) {
  llvm::Value *LHSIR = ValueAttorney::getValue(LHS);
  llvm::Value *RHSIR = ValueAttorney::getValue(RHS);
  llvm::Instruction *BeforeIR = InsertBefore->getTopmostIRInstruction();
  auto &Builder = SBCtx.getLLVMIRBuilder();
  Builder.SetInsertPoint(BeforeIR);
  auto *NewV = Builder.CreateCmp(Pred, LHSIR, RHSIR, Name, FPMathTag);
  if (auto *NewCI = dyn_cast<llvm::CmpInst>(NewV))
    return SBCtx.createCmpInst(NewCI);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}

Value *CmpInst::create(llvm::CmpInst::Predicate Pred, Value *LHS, Value *RHS,
                       BasicBlock *InsertAtEnd, Context &SBCtx,
                       const Twine &Name, MDNode *FPMathTag) {
  llvm::Value *LHSIR = ValueAttorney::getValue(LHS);
  llvm::Value *RHSIR = ValueAttorney::getValue(RHS);
...
BranchInst *BranchInst::create(BasicBlock *IfTrue, Instruction *InsertBefore,
  ...
  return SBCtx.createBranchInst(NewBr);
}
...
StoreInst *StoreInst::create(Value *V, Value *Ptr, MaybeAlign Align,
  ...
  auto *NewSBI = SBCtx.createStoreInst(NewSI);
  return NewSBI;
}
StoreInst *StoreInst::create(Value *V, Value *Ptr, MaybeAlign Align,
                             BasicBlock *InsertAtEnd, Context &SBCtx) {
...
LoadInst *LoadInst::create(Type *Ty, Value *Ptr, MaybeAlign Align,
  ...
  auto *NewSBI = SBCtx.createLoadInst(NewLI);
  return NewSBI;
}
...
Value *CastInst::create(Type *Ty, Opcode Op, Value *Operand,
  ...
  if (auto *NewCI = dyn_cast<llvm::CastInst>(NewV))
    return SBCtx.createCastInst(NewCI);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *PHINode::create(Type *Ty, unsigned NumReservedValues,
                       Instruction *InsertBefore, Context &SBCtx,
                       const Twine &Name) {
  llvm::Instruction *InsertBeforeIR = InsertBefore->getTopmostIRInstruction();
  llvm::PHINode *NewPHI =
      llvm::PHINode::Create(Ty, NumReservedValues, Name, InsertBeforeIR);
  return SBCtx.createPHINode(NewPHI);
}
...
Value *SelectInst::create(Value *Cond, Value *True, Value *False,
  ...
  if (auto *NewSI = dyn_cast<llvm::SelectInst>(NewV))
    return SBCtx.createSelectInst(NewSI);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *BinaryOperator::create(Instruction::Opcode Op, Value *LHS, Value *RHS,
  ...
  if (auto *NewBinOp = dyn_cast<llvm::BinaryOperator>(NewV)) {
    return SBCtx.createBinaryOperator(NewBinOp);
  }
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *BinaryOperator::createWithCopiedFlags(Instruction::Opcode Op, Value *LHS,
                                             ...
                                             Context &SBCtx,
                                             const Twine &Name) {
  Value *NewV = create(Op, LHS, RHS, InsertBefore, SBCtx, Name);
...
Value *BinaryOperator::createWithCopiedFlags(Instruction::Opcode Op, Value *LHS,
                                             ...
                                             Context &SBCtx,
                                             const Twine &Name) {
  Value *NewV = create(Op, LHS, RHS, InsertAtEnd, SBCtx, Name);
  llvm::Value *IRCopyFrom = ValueAttorney::getValue(CopyFrom);
  if (isa<BinaryOperator>(NewV))
    cast<llvm::BinaryOperator>(ValueAttorney::getValue(NewV))
        ->copyIRFlags(IRCopyFrom);
...
Value *UnaryOperator::createWithCopiedFlags(Instruction::Opcode Op, Value *OpV,
                                            Value *CopyFrom,
                                            Instruction *InsertBefore,
                                            Context &SBCtx, const Twine &Name) {
  ...
  if (auto *NewBinOp = dyn_cast<llvm::UnaryOperator>(NewV)) {
    ...
    return SBCtx.createUnaryOperator(NewBinOp);
  }
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *RetInst::create(Value *RetVal, Instruction *InsertBefore,
                       Context &SBCtx) {
  ...
  if (auto *NewRI = dyn_cast<ReturnInst>(NewV))
    return SBCtx.createRetInst(NewRI);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}

Value *RetInst::create(Value *RetVal, BasicBlock *InsertAtEnd, Context &SBCtx) {
  llvm::Value *LLVMRet =
...
CallInst *CallInst::create(FunctionType *FTy, Value *Func,
                           ...
                           BasicBlock *WhereBB, Context &SBCtx,
                           const Twine &NameStr) {
  ...
  return SBCtx.createCallInst(NewCI);
}

CallInst *CallInst::create(FunctionType *FTy, Value *Func,
                           ArrayRef<Value *> Args, Instruction *InsertBefore,
                           Context &SBCtx, const Twine &NameStr) {
  return CallInst::create(FTy, Func, Args, InsertBefore->getIterator(),
                          InsertBefore->getParent(), SBCtx, NameStr);
}

CallInst *CallInst::create(FunctionType *FTy, Value *Func,
                           ArrayRef<Value *> Args, BasicBlock *InsertAtEnd,
                           Context &SBCtx, const Twine &NameStr) {
  return CallInst::create(FTy, Func, Args, InsertAtEnd->end(), InsertAtEnd,
                          SBCtx, NameStr);
}
...
Value *GetElementPtrInst::create(Type *Ty, Value *Ptr,
  ...
  if (auto *NewGEP = dyn_cast<llvm::GetElementPtrInst>(NewV))
    return SBCtx.createGetElementPtrInst(NewGEP);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *InsertElementInst::create(Value *Vec, Value *NewElt, Value *Idx,
  ...
  if (auto *NewInsert = dyn_cast<llvm::InsertElementInst>(NewV))
    return SBCtx.createInsertElementInst(NewInsert);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *ExtractElementInst::create(Value *Vec, Value *Idx,
  ...
  if (auto *NewExtract = dyn_cast<llvm::ExtractElementInst>(NewV))
    return SBCtx.createExtractElementInst(NewExtract);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
Value *ShuffleVectorInst::create(Value *V1, Value *V2, Value *Mask,
  ...
  if (auto *NewShuffleVec = dyn_cast<llvm::ShuffleVectorInst>(NewV))
    return SBCtx.createShuffleVectorInst(NewShuffleVec);
  assert(isa<llvm::Constant>(NewV) && "Expected constant");
  return SBCtx.getOrCreateConstant(cast<llvm::Constant>(NewV));
}
...
void BasicBlock::buildSBBasicBlockFromIR(llvm::BasicBlock *BB) {
...
BasicBlock::BasicBlock(llvm::BasicBlock *BB, Context &SBCtx)
...
BasicBlock::iterator::getI(llvm::BasicBlock::iterator It) const {
  Instruction *SBI = cast_or_null<Instruction>(SBCtx->getValue(&*It));
  assert(
