...
class SeedBundle : public SBValBundle {
  /// The lanes that we have already vectorized.
  BitVector UsedLanes;

public:
  explicit SeedBundle(SBValBundle &&Bndl) : SBValBundle(std::move(Bndl)) {}
  explicit SeedBundle(const SBValBundle &Bndl) : SBValBundle(Bndl) {}
  ...
  virtual ~SeedBundle() {}
  using iterator = SBValBundle::iterator;
  using const_iterator = SBValBundle::const_iterator;
  ...
  SBInstruction *operator[](unsigned Idx) const {
    return cast<SBInstruction>(Vals[Idx]);
  }
  ...
  void setUsed(SBValue *SBV) {
    auto It = find(Vals, SBV);
    assert(It != Vals.end() && "SBV not in the bundle!");
  ...
  SBValBundle getSlice(unsigned Offset, unsigned MaxVecRegBits,
...
public:
  explicit MemSeedBundle(SBValBundle &&Bndl) : SeedBundle(std::move(Bndl)) {
    assert(
        all_of(*this, [](auto *SBV) { return isa<LoadOrStoreT>(SBV); }) &&
  ...
  bool tryInsert(SBInstruction *SBI, const DataLayout *DL,
    ...
    auto Cmp = [DL, SE](SBValue *SBV1, SBValue *SBV2) {
      return SBUtils::comesBeforeInMem(cast<LoadOrStoreT>(SBV1),
                                        cast<LoadOrStoreT>(SBV2), *SE, *DL);
    };
    // Find the first element after TSI in mem. Then insert TSI before it.
    auto WhereIt = std::upper_bound(Vals.begin(), Vals.end(), SBI, Cmp);
    Vals.insert(WhereIt, SBI);
    return true;
  }
};
...
public:
  SeedCollector(SBBasicBlock *SBBB, const DataLayout &DL,
                ScalarEvolution &SE);
  ~SeedCollector();
