...
;
; The new pass has BETTER KmCnt scheduling than the old pass. The old pass
; emits s_wait_kmcnt 0 early, blocking until both SMEM loads (LDS base and
; loop bound) complete before issuing any DS loads. The new pass waits
; incrementally: s_wait_kmcnt 1 before using s1 (LDS base), then allows DS
; loads to issue while s0 (loop bound) is still loading, and finally
; s_wait_kmcnt 0 right before the loop needs s0. This allows DS loads to
; execute in parallel with the remaining SMEM latency.
