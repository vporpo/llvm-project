...
; RUN: llc -mtriple=amdgpu9.0a-amd-amdhsa -stop-after=si-insert-waitcnts -amdgpu-enable-new-insert-waitcnts=false < %s | FileCheck %s
; RUN: llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx90a -stop-after=amdgpu-new-insert-waitcnts -amdgpu-enable-new-insert-waitcnts=true < %s | FileCheck %s
...
define amdgpu_kernel void @widget(ptr addrspace(1) %arg, i1 %arg1) #0 {
  ...
  ; CHECK-NEXT:   S_CMP_LG_U32 killed renamable $sgpr6, 1, implicit-def $scc
  ; CHECK-NEXT:   S_CBRANCH_SCC0 %bb.10, implicit killed $scc
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.4.bb6:
