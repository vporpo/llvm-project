...
; RUN: llc -mtriple=amdgcn--amdhsa -mcpu=kaveri -amdgpu-enable-new-insert-waitcnts < %s | FileCheck -enable-var-scope -check-prefix=GCN %s
; RUN: llc -mtriple=amdgcn--amdhsa -mcpu=fiji -mattr=-flat-for-global -amdgpu-enable-new-insert-waitcnts < %s | FileCheck -enable-var-scope -check-prefix=GCN %s
; RUN: llc -mtriple=amdgcn--amdhsa -mcpu=gfx900 -amdgpu-enable-new-insert-waitcnts < %s | FileCheck -enable-var-scope -check-prefixes=GCN,GFX9 %s
