...
; RUN: opt -passes=sandbox-vectorizer -mtriple=x86_64-- -mattr=+sse4.1 -sbvec-force-seeds=stores %s -S | FileCheck %s --check-prefix=VECTOR-SSE4
...
; VECTOR-SSE4-NEXT:    [[PACK:%.*]] = insertelement <4 x float> poison, float [[LD0]], i64 0
; VECTOR-SSE4-NEXT:    [[PACK1:%.*]] = insertelement <4 x float> [[PACK]], float [[LD1]], i64 1
; VECTOR-SSE4-NEXT:    [[PACK2:%.*]] = insertelement <4 x float> [[PACK1]], float [[LD0]], i64 2
; VECTOR-SSE4-NEXT:    [[PACK3:%.*]] = insertelement <4 x float> [[PACK2]], float [[LD1]], i64 3, !sb [[META1]]
; VECTOR-SSE4-NEXT:    [[VEC:%.*]] = fmul <4 x float> [[PACK3]], [[PACK7]], !sb [[META1]]
