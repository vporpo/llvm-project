...
/// Check that sandboxir::BasicBlock is registered with LLVMValueToSBValueMap
...
bb1:
  ...
  auto DoRAWIf = [&]() {
    SBLd0->replaceUsesWithIf(
        SBLd1, [](sandboxir::User *DstU, unsigned OpIdx) { return true; });
  };
...
define void @foo(ptr %ptr, i8 %val) {
  ...
  sandboxir::Instruction *Ret = &SBBB.back();
...
bb1:
  ...
  sandboxir::Instruction *Br = &BB0.back();
