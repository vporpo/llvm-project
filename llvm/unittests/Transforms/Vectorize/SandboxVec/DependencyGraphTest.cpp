...
define void @foo(float %v1, float %v2) {
  ...
  DAG.extend(SBValBundle{&*BB->begin(), BB->getTerminator()->getPrevNode()});
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *Ld2 = &*It++;
  auto *Add = &*It++;
  DAG.extend(SBValBundle{Ld1, Add});
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *St1 = &*It++;
  auto *St2 = &*It++;
  DAG.extend(SBValBundle{St1, St2});
...
define void @foo(float %v, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DependencyGraph DAG(Ctxt, AA);
  SBBasicBlock *BB = Ctxt.createSBBasicBlock(&*F.begin());
  DAG.extend(SBValBundle{&BB->front(), &BB->back()});
...
define void @foo(i8 %v, ptr noalias %ptr1, ptr noalias %ptr2, ptr noalias %ptr3, ptr noalias %ptr4) {
  ...
  DependencyGraph DAG(Ctxt, AA, /*Sched=*/nullptr, /*AAQueryBudget=*/1);
  DAG.extend(SBValBundle{&*BB->begin(), BB->getTerminator()->getPrevNode()});
...
define void @foo(i8 %v0, i8 %v1, i8 %v2, i8 %v3, ptr noalias %ptrA, ptr noalias %ptrB) {
  ...
  {
    // AA Budget = 9999
    DependencyGraph DAG(Ctxt, AA, /*Sched=*/nullptr,
                              /*AAQueryBudget=*/9999);
    DAG.extend(
        SBValBundle{&*BB->begin(), BB->getTerminator()->getPrevNode()});
    auto It = BB->begin();
    auto *St0 = &*It++;
    auto *St1 = &*It++;
    auto *St2 = &*It++;
  ...
  {
    // AA Budget = 0
    DependencyGraph DAG(Ctxt, AA, /*Sched=*/nullptr, /*AAQueryBudget=*/0);
    DAG.extend(
        SBValBundle{&*BB->begin(), BB->getTerminator()->getPrevNode()});
    auto It = BB->begin();
    auto *St0 = &*It++;
...
define void @foo(float %v, float %v0, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DAG.extend(SBValBundle{Ld2, Ld2});
  expect_deps(DAG, Ld2, {});

  DAG.extend(SBValBundle{Ld1, Ld1});
  expect_deps(DAG, Ld1, {});
  expect_deps(DAG, Ld2, {});

  DAG.extend(SBValBundle{St1, St1});
  expect_deps(DAG, Ld1, {});
  expect_deps(DAG, Ld2, {});
  expect_deps(DAG, St1, {Ld1});

  DAG.extend(SBValBundle{St2, St2});
  expect_deps(DAG, Ld1, {});
  expect_deps(DAG, Ld2, {});
  expect_deps(DAG, St1, {Ld1});
  expect_deps(DAG, St2, {Ld2});

  DAG.extend(SBValBundle{St0, St0});
  expect_deps(DAG, St0, {});
...
define void @foo(float %v, float %v0, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DAG3.extend(SBValBundle{Ld2, Ld2});
  expect_deps(DAG3, Ld2, {});

  DAG3.extend(SBValBundle{St0});
  DAG3.extend(SBValBundle{St2});
  ...
  DAG4.extend(SBValBundle{St0, St2});
  ...
  DAG4.extend(SBValBundle{Ld2, Ld2});
  expect_deps(DAG4, St0, {});
  expect_deps(DAG4, Ld1, {St0});
  expect_deps(DAG4, Ld2, {});
  expect_deps(DAG4, St1, {Ld1, St0});
...
define void @foo(float %v1, float %v2, ptr %ptr) {
  ...
  DAG.extend(SBValBundle{St2});

  DAG.extend(SBValBundle{Ld1, DepLoad});
  auto *Ld1N = DAG.getNode(Ld1);
  auto *St1N = DAG.getNode(St1);
  auto *DepLoadN = DAG.getNode(DepLoad);
  auto *St2N = DAG.getNode(St2);
...
define void @foo(ptr %ptr) {
  ...
  {
    auto Rgn = DAG.extend(SBValBundle{St1});
    EXPECT_TRUE(Rgn == InstrRange(St1, St1));
  }
  {
    auto Rgn = DAG.extend(SBValBundle{St0});
    EXPECT_TRUE(Rgn == InstrRange(St0, St0));
  }
  {
    DAG.resetView();
    auto Rgn = DAG.extend(SBValBundle{Ld0, Ret});
    EXPECT_TRUE(Rgn == InstrRange(Ld0, Ret));
  }
}
...
define void @foo(ptr noalias %ptr0, ptr noalias %ptr1) {
  ...
  DAG.extend(SBValBundle{Ld0, Ld1, St0, St1});
  DAG.resetView();
...
loop:
...

  {
    ...
    DAG.extend({Add0});
    DAG.extend({PHI});
    DAG.extend(SBValBundle{Ld0, BB->getTerminator()});
    auto *PHIN = DAG.getNode(PHI);
    for (SBInstruction &I : drop_begin(*BB)) {
      auto *N = DAG.getNode(&I);
      EXPECT_FALSE(dependencyPath(PHIN, N));
...
define void @foo(float %v1, float %v2, float %v3, ptr %ptr) {
  ...
  DAG.insertAndAddDeps(St1);
  auto *S1 = DAG.getNode(St1);
  DAG.insertAndAddDeps(St3);
  auto *S3 = DAG.getNode(St3);
  ...
  DAG.insertAndAddDeps(St2);
  auto *S2 = DAG.getNode(St2);
  EXPECT_TRUE(S2->dependsOn(S1));
  EXPECT_TRUE(S3->dependsOn(S2));
}

// Checks DAG.insertAndAddDeps(). The new instruction is in the DAG region.
TEST(DependencyGraph, DAGInsertAndAddDeps_InInstrRange) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
...
// Checks DAG.insertAndAddDeps() when adding an instruction not in the DAG.
TEST(DependencyGraph, DAGInsertAndAddDeps_NotInInstrRange) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
define void @foo(float %v1, float %v2, float %v3, float %v4, <2 x float> %vec,
                 ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  auto *St4 = &*It++;
  auto *Ret = &*It++;
  DAG.extend(SBValBundle{St1, St4});
...
// Checks DAG.insertAndAddDeps() when adding an instr that has a dependency with
// an instr in the DAGRange but not in the current ViewRange.
//
// DAG  View
//  -     -
//  |     |  o New
...
TEST(DependencyGraph,
     DAGInsertAndAddDeps_DependencyOutsideView_DepBelow) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
define void @foo(float %v1, float %v2, float %v3, float %new, ptr noalias %ptr1, ptr noalias %ptr2) {
  ...
  DAG.extend(SBValBundle{St1, St3});
  ...
  for (auto *SBI : {St1, St2, St3})
    EXPECT_TRUE(DAG.getDAGRange().contains(SBI));
...
//  -     -
TEST(DependencyGraph,
     DAGInsertAndAddDeps_DependencyOutsideView_DepAbove) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
define void @foo(float %v1, float %v2, float %v3, float %new, ptr noalias %ptr1, ptr noalias %ptr2) {
  store float %v1, ptr %ptr1
  ...
  DAG.resetView();
  EXPECT_TRUE(DAG.getView().empty());
  DAG.extend(SBValBundle{St2, St3});
  // Check the regions.
...
// Checks DAG.insertAndAddDeps() when adding an instr outside the View that has
...
//  -
TEST(DependencyGraph,
     DAGInsertAndAddDeps_NewAndDepOutsideView_NewAboveDep) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
define void @foo(float %v1, float %v2, float %new, ptr noalias %ptr1, ptr noalias %ptr2) {
  store float %v1, ptr %ptr1
...
TEST(DependencyGraph, NotifyErase) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
define void @foo(ptr %ptr, i8 %v) {
  ...
  DAG.extend(SBValBundle{Add, Ret});
  DAG.notifyRemove(St);
  EXPECT_EQ(DAG.getNode(St), nullptr);
}
...
define void @foo(ptr %ptr, i8 %B, i8 %A, i8 %N) {
  ...
  DAG.extend(SBValBundle{StB, StA});
  ...
  // Now remove the transient node "N"
  DAG.notifyRemove(N->getInstruction());
...
define void @foo(ptr noalias %ptr, i8 %A) {
  ...
  DAG.extend(SBValBundle{LdB, StA});
...
define void @foo(ptr noalias %ptr0, ptr noalias %ptr1, i8 %v) {
  ...
  DAG.extend(SBValBundle{St0, Ret});
...
define void @foo(ptr %ptr, i8 %v1, i8 %v2, i8 %v3) {
  ...
  DAG.extend(SBValBundle{Add1, Ret});
