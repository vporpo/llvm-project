...
define void @foo(ptr noalias %ptr) {
  ...
  EXPECT_EQ(*B2, *B2);
  B2->remove(NB2);
  EXPECT_EQ(*B2, SchedBundle({NB1}, Sched));
...
define void @foo(ptr noalias %ptr1, ptr noalias %ptr2, ptr noalias %ptr3,
  ...
  auto *NewI = SBStoreInstruction::create(Ld0, Ptr, /*Align=*/std::nullopt,
                                            Add0, Ctxt);
...
bb1:
  ...
  DAG.extend(SBValBundle{&*BB1->begin(), BB1->getTerminator()});
  Scheduler &Sched = *Ctxt.getScheduler(BB1);
  auto &ReadyList = const_cast<ReadyListContainer &>(Sched.getReadyList());
...
define void @foo(ptr %ptr, <2 x i8> %vec) {
  ...
  SchedulerAttorney::extendRegionAndUpdateReadyList(
      Sched, SBValBundle{&*BB->begin(), St1});
  EXPECT_TRUE(Sched.trySchedule({St0, St1}));
...
define void @foo(ptr %ptr, i8 %val) {
  ...
  const InstrRange &View = Ctxt.getDAG(BB).getView();
  const InstrRange &DAGRange = Ctxt.getDAG(BB).getDAGRange();
  ...
  EXPECT_EQ(DAGRange.from(), Ret);
  EXPECT_EQ(DAGRange.to(), Ret);
  ...
  EXPECT_EQ(DAGRange.from(), NewSBI);
  EXPECT_EQ(DAGRange.to(), NewSBI);

  NewSBI->eraseFromParent();
  EXPECT_EQ(View.from(), nullptr);
  EXPECT_EQ(View.to(), nullptr);
  EXPECT_EQ(DAGRange.from(), nullptr);
  EXPECT_EQ(DAGRange.to(), nullptr);
}
...
define void @foo(ptr %ptr) {
  ...
  SBValBundle NewBndl({Ld1, Ld0});
  EXPECT_EQ(
      SchedulerAttorney::getBndlSchedState(Sched, NewBndl),
...
define void @foo(ptr %ptr, i8 %v0, i8 %v1) {
  ...
  auto *St0 = SBStoreInstruction::create(ArgV0, Ptr, /*Align=*/std::nullopt,
                                           St1, Ctxt);
  auto *St0N = DAG.getNode(St0);
...
define void @foo(ptr %ptr, i8 %v0, i8 %v1) {
  ...
  auto *St0N = DAG.getNode(St0);
  EXPECT_TRUE(St0N->isScheduled());
  // St0N gets scheduled without its successors being scheduled because we
  // force-insert it.
...
define void @foo(float %v0, ptr %ptr) {
  ...
  auto *St = &*It++;
  auto *Ret = &*It++;
  DAG.extend(SBValBundle{Add, BB->getTerminator()});
...
TEST(Scheduler,
     DAGInsertAndAddDeps_UpdateUnschedSuccsOfPreds_MemAndUseDef) {
  LLVMContext C;
  std::unique_ptr<Module> M = parseIR(C, R"IR(
...
define void @foo(float %v0, float %v1, ptr noalias %ptr0, ptr noalias %ptr1, float %vNew) {
  ...
  auto *St0 = &*It++;
  auto *St1 = &*It++;
  DAG.extend(SBValBundle{St0, BB->getTerminator()});
...
define void @foo(ptr %ptr, i8 %v0, i8 %v1, i8 %v2) {
  ...
  auto *St1 = &*It++;
  auto *St2 = &*It++;
  DAG.extend(SBValBundle{St0, St2});
  auto *St0N = DAG.getNode(St0);
  auto *St1N = DAG.getNode(St1);
